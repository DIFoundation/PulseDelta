...
export const EVENT_SIGNATURES = {
  MarketCreated:
    "0x1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b", // recompute exact topic
};
...
export const useMultiFactoryUtils = () => {
  ...
  const validateTimeRange = (
    ...
    if (endTime >= resolutionDeadline)
      return { valid: false, error: "End must be before resolution deadline" };
    if (startTime <= now)
      return { valid: false, error: "Start must be in future" };
