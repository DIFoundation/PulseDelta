...
import "../MarketBinary.sol";
import "../wDAG.sol";
import "../FeeRouter.sol";
import "../IOracleAdapter.sol";

contract BinaryMarketFactory {
    ...
    function getMarketsByStatus(
        ...
        for (uint256 i = 0; i < allMarkets.length; i++) {
            try MarketBinary(allMarkets[i]).state() returns (
                MarketBinary.State marketState
            ) {
                ...
                // Skip markets that fail to return state
                continue;
            }
        }
    ...
    function getMarketsByStatusPaged(
        uint8 status,
        uint256 offset,
        uint256 limit
    ) external view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < resultLength; i++) {
            result[i] = filteredMarkets[start + i];
        }
