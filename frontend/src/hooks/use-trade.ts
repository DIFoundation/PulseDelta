...
export function useTrade(marketId: string, outcomeIndex: number) {
	...
	const executeTrade = useCallback(
		async (params: TradeParams) => {
			...
			const estimatedShares = type === "buy" ? amount / marketState.data.outcomePrice : amount
			const cost =
				type === "buy" ? amount + fee : amount * marketState.data.outcomePrice - fee
			...
			try {
				...
				const optimisticUpdate = {
					...
					marketLiquidity:
						marketState.data.marketLiquidity + (type === "buy" ? amount : -amount),
				}
				...
				addToast({
					...
					description: `${type === "buy" ? "Bought" : "Sold"} ${estimatedShares.toFixed(
						2
					)} shares`,
					type: "success",
					duration: 5000,
				...
				rollbackOptimistic(error instanceof Error ? error.message : "Trade failed", {
					balance: 1234.56,
					shares: [0, 0],
					marketLiquidity: 125000,
					outcomePrice: 0.65,
				...
				failTransaction(error instanceof Error ? error.message : "Trade failed")
