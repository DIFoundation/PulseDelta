...
export function useBinaryFactoryGetters() {
  ...
  const useGetMarketsByStatusPaged = (
    ...
    return useReadContract({
      ...
      args:
        status !== undefined && offset !== undefined && limit !== undefined
          ? [status, offset, limit]
          : undefined,
      query: {
        enabled:
...
export const useBinaryFactoryUtils = () => {
  ...
  const validateTimeRange = (
    ...
    if (endTime >= resolutionDeadline) {
      return {
        valid: false,
        error: "End time must be before resolution deadline",
      };
    }
  ...
  const formatMarketParams = (
    params: Partial<MarketCreateParams>
  ): MarketCreateParams | null => {
    if (
      !params.collateral ||
      !params.question ||
      !params.creator ||
