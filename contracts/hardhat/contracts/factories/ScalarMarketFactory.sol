...
import "../MarketScalar.sol";
import "../wDAG.sol";
import "../FeeRouter.sol";
import "../IOracleAdapter.sol";

contract ScalarMarketFactory {
    ...
    function createScalar(
        ...
        market = address(
            new MarketScalar(
                ...
                feeBps,
                msg.value,
                times // Use the DAG amount as liquidity
            )
        );
    ...
    function getMarketsByStatus(
        ...
        for (uint256 i = 0; i < allMarkets.length; i++) {
            try MarketScalar(allMarkets[i]).state() returns (
                MarketScalar.State marketState
            ) {
